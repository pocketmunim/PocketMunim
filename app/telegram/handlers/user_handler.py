...
class UserHandler:
    ...
    async def security_check(supabase_admin, chat_id, text):
        if MALICIOUS_PATTERN.search(text):
            try:
                ...
                supabase_admin.table('users').update({'security_strikes': new_strikes}).eq('telegram_id',
                                                                                           chat_id).execute()
                if new_strikes >= 3:
                    await send_telegram_reply(chat_id, "🛑 *ACCOUNT SUSPENDED*\n\nMultiple malicious inputs detected.")
                else:
    ...
    async def prompt_registration(chat_id):
        ...
        await send_telegram_reply(chat_id,
                                  f"📝 *Registration Mandatory*\n\nTo use PocketMunim, register first:\n\n{copyable_form}")
    ...
    async def register(supabase_admin, chat_id, user_id, text, user_exists):
        ...
        if not name or monthly_salary is None or not bank_name or current_balance is None:
            await send_telegram_reply(chat_id, "⚠️ *Registration Failed*\n\nMissing required fields.")
            return
        ...
        if not user_exists:
            try:
                ...
                await send_telegram_reply(chat_id, f"⚠️ Registration failed:\n`{str(e)}`")
        else:
