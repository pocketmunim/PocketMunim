...
class CallbackHandler:
    ...
    async def handle(payload: dict, supabase_admin):
        ...
        if data.startswith("btog_"):
            ...
            if batch and "items" in batch:
                ...
                if not selected_items:
                    await edit_telegram_message(chat_id, message_id, text="  No duplicates selected. Discarded.")
                else:
            ...
            batch_id = data.split("_")[1]
            batch_dao.delete_batch(batch_id)
            await edit_telegram_message(chat_id, message_id, text="  Duplicate batch discarded.")
        ...
        elif data.startswith("payemiadv_"):
            loan_id = data.split("_")[1]
            from app.services.loan_service import LoanService
            service = LoanService(supabase_admin, user_id)
            result_msg, _ = await service.process_emi_payment_by_id(loan_id, skip_month_check=True)
            await edit_telegram_message(chat_id, message_id, text=result_msg)

        elif data == "cancel_action":
            await edit_telegram_message(chat_id, message_id, text="  Action Cancelled.")
