        ...
        if data.startswith("btog_"):
            ...
            if batch and "items" in batch:
                ...
                if not selected_items:
                    await edit_telegram_message(chat_id, message_id, text="⚠️ No duplicates selected. Batch discarded.")
                else:
                    ...
                    if acc_res.data:
                        ...
                        total_deduction = sum(p["amount"] for p in selected_payloads if p.get("source_account") == default_acc_name)
                        total_addition = sum(p["amount"] for p in selected_payloads if p.get("destination_account") == default_acc_name)
